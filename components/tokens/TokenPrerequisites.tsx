        ...
        {!items.length ? (
          ...
          items.map((item, idx1) => (
            <Fragment key={`prerequisite-${idx1}`}>
              <Link href={item.purchaseUrl} target='_blank' rel='noopener noreferrer' className={!multiItemType ? 'mx-1' : ''}>
                <ImageLoader src={item.imageUrl} alt='nft' width={150} height={150} style={{ borderRadius: '1rem' }} />
